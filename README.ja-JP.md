...
- **デコピン** — `Settings…` → `General` で有効化。Ctrl（macOS では Option）を押しながら、飛ばしたい方向とは**逆**にドラッグします。Clawd はその場に留まり、照準線が着地点を示し、離すと発射されます。強く引けば別のディスプレイまで飛びます。強さは調整可能
...
Clawd はマルチモニター環境に適応します。起動したディスプレイに合わせて比例サイズを決め、縦長モニターでは読みやすさを保つために上限付きで拡大し、ディスプレイ間のドラッグにも対応します。強めのデコピンなら、別のディスプレイまで飛ばすこともできます。
