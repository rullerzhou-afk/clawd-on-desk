...
(function initSettingsTabMobile(root) {
  ...
  function renderConnectionInfo(container, attempt = 0) {
    ...
    fetchMobileInfo().then((info) => {
      ...
      // QR code — scan from the phone camera (same Wi-Fi) to pair instantly.
      // The SVG is generated by the trusted main process (no user HTML).
      if (info.qrSvg) {
        html += '<div class="mobile-qr" style="text-align:center;margin:4px 0 14px;">'
          + `<span style="display:inline-block;background:#fff;padding:12px;border-radius:10px;line-height:0;">${info.qrSvg}</span>`
          + '</div>';
      }
