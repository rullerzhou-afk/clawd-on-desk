...
// Registered in CodeBuddy settings by hooks/codebuddy-install.js
...
// ─── Process tree walk (zero-dependency, duplicated per hook script) ────────

const TERMINAL_NAMES_WIN = new Set([
  "windowsterminal.exe", "cmd.exe", "powershell.exe", "pwsh.exe",
  "code.exe", "codebuddy.exe", "alacritty.exe", "wezterm-gui.exe",
  "mintty.exe", "conemu64.exe", "conemu.exe", "hyper.exe", "tabby.exe",
...
const EDITOR_MAP_LINUX = { "code": "code", "cursor": "cursor", "codebuddy": "codebuddy", "code-insiders": "code" };

const CODEBUDDY_NAMES_WIN = new Set(["codebuddy.exe", "CodeBuddy.exe"]);
const CODEBUDDY_NAMES_MAC = new Set(["CodeBuddy"]);
const CODEBUDDY_NAMES_LINUX = new Set(["codebuddy", "CodeBuddy"]);
...
// CodeBuddy gating hooks need stdout JSON response
...
// ─── Main: read stdin JSON, extract event, post state, write stdout ─────────
