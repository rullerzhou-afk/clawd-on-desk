...
function registerQoderHooks(options = {}) {
  ...
  const hookScript = asarUnpackedPath(path.resolve(__dirname, "qoder-hook.js").replace(/\\/g, "/"));
  ...
  for (const event of QODER_HOOK_EVENTS) {
    ...
    const result = normalizeQoderHookEntries(arr, desiredCommand);
    ...
    arr.push(buildQoderHookEntry(desiredCommand));
    added++;
    changed = true;
  }
    ...
    for (const entry of arr) {
      ...
      if (Array.isArray(entry.hooks)) {
        ...
        if (otherHooks.length !== entry.hooks.length) {
          changed = true;
          if (otherHooks.length === 0) continue; // remove entire entry
          entry.hooks = otherHooks;
        }
      }
      next.push(entry);
