...
// REMOVED: ACTIVE_STATES_BLOCKING_THINKING gate.
// The server-side state machine (state.js) already handles priority correctly
// (working > thinking), so a working→thinking transition is harmless — state.js
// will keep the higher-priority working state. The old gate incorrectly blocked
// legitimate thinking events that fire after tool completion in the opencode/
// CodeFree-O event model (session.status busy → thinking after PostToolUse).
...
function sendState(state, eventName, sessionId) {
  ...
  // Subagent sessions (spawned by the `task` tool) get headless=true so
  // the Dashboard and Session HUD don't show duplicate cards for the same
  // conversation. They still participate in the multi-session state machine
  // (1→typing, 2→juggling, 3+→building) for the pet animation.
...
// properties (only `id` = requestID), so we use _rootSessionId (the primary
// user-facing session) as the best guess. Phase 1 dedup/state machine logic
// does not run for permission events — they ride a parallel channel and never
// translate to a Clawd state transition.
function handlePermissionAsked(event) {
  ...
  postPermissionToClawd({
    ...
    session_id: _rootSessionId || "default",
    ...
    event: async ({ event }) => {
      try {
        ...
        if (event.type === "session.created") {
          debugLog(`DIAG session.created props.sessionID=${event.properties && event.properties.sessionID} event.sessionID=${event.sessionID} sid=${sid} _root=${_rootSessionId}`);
        }
