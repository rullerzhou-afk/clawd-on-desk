...
| **安装包自动更新仍需用户确认** | Clawd 不会在后台自行重启，因为这可能中断正在运行的 agent 权限 hooks。Windows NSIS、macOS DMG、Linux AppImage 和 Debian/Ubuntu deb 都可在用户确认后更新；AppImage/deb 以外的 Linux 包仍需通过对应包管理器更新。 |
