...
| **Kimi Code: `command` field is owned by Clawd** | Clawd auto-syncs Kimi hooks on every startup (and from `npm run install:kimi-hooks`). For each `[[hooks]]` block whose `name`/`event` Clawd recognizes, the `command` field is rewritten to the canonical line (including any `CLAWD_KIMI_PERMISSION_MODE=…` prefix derived from the env at install time). Manual edits to that `command` field will be silently overwritten on the next launch — change behavior via env vars (e.g. `CLAWD_KIMI_PERMISSION_MODE`) before re-running the installer instead of editing the TOML by hand. |
