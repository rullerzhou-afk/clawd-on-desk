...
**OMP (oh-my-pi)** — uses a per-agent extension directory: `~/.omp/agent/extensions/clawd-on-desk` in the default environment. Install it from **Settings → Agents** when you want local OMP tracking; after that Clawd keeps the extension synced on launch while OMP remains enabled. You can also run `node hooks/omp-install.js` manually, and `node hooks/omp-install.js --uninstall` to remove it. Clawd resolves the directory the way OMP itself does — `PI_CONFIG_DIR` moves the config root, `PI_CODING_AGENT_DIR` relocates the profile-less default, and an active `OMP_PROFILE` / `PI_PROFILE` selects `~/.omp/profiles/<name>/agent` — so the install lands in the directory OMP will load for **Clawd's own environment**. OMP running under a *different* profile than Clawd loads nothing from it; run the manual installer from that profile's shell to add one, and Doctor will name the profiles it does not manage. Interactive OMP sessions report lifecycle and tool activity to Clawd, but OMP is state-only: Clawd shows no permission bubbles and preserves OMP's own execution behavior. If the community bridge `clawd-on-desk-omp.ts` is present in that extensions directory, Clawd skips installing its own extension (and retires only a copy it verifiably wrote earlier) so OMP never loads both and reports every event twice.
