...
- **彈指** — 需在 `Settings…` → `General` 開啟：按住 Ctrl（macOS 為 Option）並朝目標的**反方向**拖曳；Clawd 原地不動，瞄準線標出落點，放手即彈出——力道足時可跨螢幕。力道可調
...
Clawd 支援多螢幕場景：按啟動時所在螢幕做等比縮放，直立螢幕有尺寸加成防止寵物過小，也可以跨螢幕拖動；力道足夠的彈指同樣能把 Clawd 彈到另一塊螢幕。
