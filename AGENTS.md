...
- DeepSeek Harness 首发是 **web profile / Windows-first experimental / plugin-only**；rc.6 已于 2026-08-29 完成 macOS / Windows x64 源码运行的真实 API-backed session/approval smoke（#962）；rc.2 Windows packaged-app 证据仅覆盖安装、web boot 与直接 bridge client 的 state/Allow/Deny 往返（#938），不得合并宣称 rc.2 安装包的真实 API 会话已验证。Clawd-managed in-process plugin 监听公开 `session/created`、`session/event`、`session/disposed`、可选 `sessionProjections.onChanged` 与 blocking `approval/request`；标题和上下文仅作为会话元数据同步，不占用生命周期 seq。状态按 session FIFO 上报并携带上游 seq；Clawd 不读取 `$DSH_HOME/storages` projection。普通审批进入独立 DSH `/permission` adapter，只支持人工 Allow Once / Deny；204 / 断连 / DND / disabled 都由 plugin `next()` 交还 DSH 原生 web answerer。`ask_user_question` 始终留在 DSH 原生 provider；DSH 不进入 `KNOWN_PERMISSION_AGENTS`，auto-tools / unattended / per-session grant 全部 DEFER。安装器通过官方 `dsh plugin --profile web add/remove` 管理 immutable marker-owned generation，foreign 同名包 fail closed；当前支持一张 verified 版本契约表：优先 `@deepseek-ai/dsh@0.1.5-rc.1`（integrity `sha512-rmNmzQCg3oIc1z8xH7izRSOuy1TNzq+/NILyfM+7e8DKOyV+yBtg47WEsqR2SiIe1ATec3L/rUa1YhIcfQ2XEg==`），并保留 `@deepseek-ai/dsh@0.1.1-rc.2`（integrity `sha512-UP1UIh6q3Gme/yXRn/QL2P8IsVlv8Shpg22TRJIZPsCRWLm4CBiA1MUvXmJAfsOEETBMLAl+xWPtFw6ICsN3wg==`）与 `@deepseek-ai/dsh@0.1.0-rc.6`（integrity `sha512-brpZfED7ieRa2PQ5tUxMhHrM1pb2CmKFVM/f6yMULBDMicahk+Z2OsHgTwTDnoiZm23Ftu9rQz0NN4pflaoJcg==`）；Install/Repair 按检测到的 host 版本（无 CLI 时按 owned marker）选择契约，Uninstall / 手动 npx 命令按 marker 契约选择，未列入表的版本禁止 mutation，且预发布版本不做宽泛 range 匹配。`47f9438` 仅是源码调查基线，不代表任何 tag/commit 映射。
