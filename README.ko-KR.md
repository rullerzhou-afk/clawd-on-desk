...
- **튕기기** — `Settings…` → `General`에서 켜서 사용합니다. Ctrl(macOS에서는 Option)을 누른 채 보내려는 방향의 **반대로** 드래그하면 Clawd는 제자리에 멈춘 채 조준선이 착지 지점을 보여 주고, 놓으면 그곳으로 날아갑니다. 세게 당기면 다른 디스플레이까지 넘어갑니다. 세기는 조절할 수 있습니다.
...
Clawd는 멀티 모니터 환경에 맞춰 동작합니다: 실행된 디스플레이에 비례한 크기 조정, 세로 모니터에서는 너무 작아 보이지 않도록 크기 보정, 디스플레이 간 드래그 이동을 지원합니다. 세게 튕기면 다른 디스플레이로도 넘어갑니다.
